...
let password = "";
// 创建统一身份认证系统的令牌
let cas_token = CasToken::acquire_by_login(stu_id, password).await.unwrap();
// 通过统一身份认证系统登录来获得教务系统的令牌
let hdjw_token = HdjwToken::acquire_by_cas_login(&cas_token)
    .await
    .unwrap();
...
我们在 [`CONTRIBUTING.md`](./CONTRIBUTING.md) 中对贡献方法进行了说明，你可以参考。
