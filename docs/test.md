...
我们建议在 `.env` 中将 `TEST_CACHE` 设为 `true`，这样测试时会把拿到的登录令牌缓存到 `cache/` 目录下（按模块分文件夹）。这样的一个好处是，在测试不同的项目的时候就不用去重复发起登录操作来获取令牌了，而是直接用之前已经拿到的令牌。另一个好处是，遇到双因素认证后，你只需要输入一次手机验证码即可，hnu_query 会把已经通过双因素认证的 `CasToken` 存到本地，后续就不用再进行双因素认证了。
