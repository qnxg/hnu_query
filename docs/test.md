...
测试时请务必传递 `--nocapture` 参数。湖大的统一身份认证系统添加了双因素认证，在登陆时需要获取手机验证码，hnu_query 在遇到需要双因素认证的时候会在控制台询问是否继续，以及询问验证码，你需要进行输入。如果不开启 `--nocapture` 则无法看到输出的提示信息。

由于测试时共享了一个全局的 `reqwest` 的 client，所以测试时还需要传递 `--test-threads 1` 避免奇怪的报错。同时为了使得不会有多线程同时进行双因素认证的询问，你也需要传递这个参数。

我们建议在 `.env` 中将 `TEST_CAS_CACHE` 设为 `true`，这样测试时会把拿到的 `CasToken` 缓存到本地。这样的一个好处是，在测试不同的项目的时候就不用去重复发起登录操作来获取 `CasToken` 了，而是直接用之前已经拿到的 `CasToken`。另一个好处是，遇到双因素认证后，你只需要输入一次手机验证码即可，hnu_query 会把已经通过双因素认证的 `CasToken` 存到本地，后续就不用再进行双因素认证了。

当然设置 `TEST_CAS_CACHE` 之后，进行测试时会抛出 `Other(TokenExpired)` 这种错误，此时说明之前缓存的 CasToken 过期了。测试时会输出，比如：`使用 CasToken 缓存: 0982616....`，后面这串字符就表示测试时使用的 `cache` 文件夹下的缓存文件，你可以手动将对应的文件删除，这样再进行测试的时候就会重新获取 CasToken 并缓存到本地。
...
```bash
cargo test ca:: -- --ignored --nocapture --test-threads 1 # 测试可信电子凭证平台相关
...
```bash
cargo test wxpay:: -- --ignored --nocapture --test-threads 1
```
...
```bash
cargo test lab::login -- --ignored --nocapture --test-threads 1
cargo test lab::schedule -- --ignored --nocapture --test-threads 1
cargo test lab::semester -- --ignored --nocapture --test-threads 1
```

可以从 `lab::semester` 的输出信息中得到学期 id，将其配置到 `TEST_LAB_SEMESTER_ID` 中，就可以继续测试：

```bash
cargo test lab::course -- --ignored --nocapture --test-threads 1
```

`lab::course` 的输出中可以得到课程 id，将其配置到 `TEST_LAB_COURSE_ID` 中，就可以继续测试：

```bash
cargo test lab::grade -- --ignored --nocapture --test-threads 1
```
...
```bash
cargo test hdjw:: -- --ignored --nocapture --test-threads 1
```
