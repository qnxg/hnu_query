...
我们建议在 `.env` 中将 `TEST_CACHE` 设为 `true`，这样测试时会把拿到的登录令牌缓存到 `cache/` 目录下（目前只缓存 cas 和 cg ，按模块分文件夹）。这样的一个好处是，在测试不同的项目的时候就不用去重复发起登录操作来获取令牌了，而是直接用之前已经拿到的令牌。另一个好处是，在遇到需要输入登录验证码，或是需要进行双因素认证时，你只需要进行一次验证即可，hnu_query 会把已经通过验证的令牌信息存到本地，后续就不用再进行验证了。

当然设置 `TEST_CACHE` 之后，进行测试时会抛出 `Other(TokenExpired)` 这种错误，此时说明之前缓存的令牌过期了。测试时会输出，比如：`使用 CasToken 缓存: 0982616....`，后面这串字符就表示测试时使用的 `cache/cas/` 文件夹下的缓存文件，你可以手动将对应的文件删除，这样再进行测试的时候就会重新获取令牌并缓存到本地。
