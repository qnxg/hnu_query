use crate::{
    ...
    test::{TEST_CACHE, TEST_PASSWORD, TEST_STU_ID, TestResult, init_tracing},
};
use std::io::{Read, Write};

/// 创建一个测试用的 [CasToken]
///
/// 如果设置了 `TEST_CACHE`（详见 `docs/test.md`）
/// 这个函数会尝试从工作目录下的 `cache/cas/` 文件夹中加载之前已经缓存的 CasToken
///
/// 如果没有设置 `TEST_CACHE`，或是没有从 `cache/cas/` 中找到已经缓存的 CasToken，
/// 则登录账号，获取 CasToken，并以交互的方式来处理需要双因子认证的情况。
///
/// 通过登录获取了新的 CasToken 之后，如果设置了 `TEST_CACHE`，则会将
/// 新的 CasToken 自动缓存到 `cache/cas/` 文件夹中。
pub async fn get_cas_token() -> TestResult<CasToken> {
    ...
    if *TEST_CACHE {
        ...
        std::fs::create_dir_all("cache/cas").expect("Failed to create cache directory");
        let mut cache_file = std::fs::OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .open(format!("cache/cas/{}", cache_name))?;
