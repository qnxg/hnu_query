...
pub enum TransactionType {
    ...
    /// 充值
    Recharge,
    /// 领取补助 (电子账户 -> 校园卡), **注意: 不一定是正值**, 并且与其他记录**重复**, 交易类型语义不明确
    ///
    /// 计算总收入/支出金额时需要排除掉这个类型
    ///
    /// 例如你会遇到以下情况:
    /// - 当你给校园卡充值后, 你会收到一条 `Recharge` 类型的记录, 再在食堂刷卡机刷卡后, 会出现一条 `MoneyTransfer` 类型的记录, 两条记录的金额相同
    /// - 当你使用了洗衣机后, 你会收到多条 `Withholding` 类型的记录, 再在食堂刷卡机刷卡后, 会出现一条 `MoneyTransfer` 类型的记录, 两条记录的金额相同
    MoneyTransfer,
    /// 补助
    Benefit,
    /// 电子账户开户, 出现于新生开卡
