...
pub async fn get_electricity(dormitory: Dormitory) -> Result<String, crate::Error<Infallible>> {
    ...
    let (park, build, room) = parse::convert_wxpay_dormitory(dormitory)?;
    let raw_data = match build.as_str() {
        // 望麓桥学生公寓的2栋和3栋无法区分南边还是北面
        // 考虑到同一个宿舍号不可能既是南又是北，所以我们两个都试试，取成功的
        "#2栋" | "#3栋" => {
            let res_north = raw::check_room_no(
                park,
                match build.as_str() {
                    "#2栋" => "52",
                    "#3栋" => "54",
            ...
            .await;
            let res_south = raw::check_room_no(
                park,
                match build.as_str() {
                    "#2栋" => "53",
                    "#3栋" => "55",
        ...
        _ => raw::check_room_no(park, build.as_str(), room.as_str()).await,
    }?;

    parse::balance(raw_data)
}
